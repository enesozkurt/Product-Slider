...
function loadData(products) {
  const productsData = document.getElementById('owl-demo');
  let dataHtml = '';

  for (let product of products) {

    let availableData = `${product.categories}`;
    let desiredData = replaceCommaLine(availableData);
    function replaceCommaLine(data) {
      //convert string to array and remove whitespace
      let dataToArray = data.split(',').map(item => item.trim());
      //convert array to string replacing comma with |
      return dataToArray.join(" | ");
    }

    let isNew = `${product.params.isNew}`;
    let desiredDataIsNew = replaceCommaLineIsNew(isNew);
    function replaceCommaLineIsNew(data) {
      //convert string to array and remove whitespace
      let dataToArray = data.split('true').map(item => item.trim());
      //convert array to string replacing comma with |
      return dataToArray.join("NEU");
    }

    dataHtml += `<div>
<div class="card">
  <img src=${product.image} alt=${product.name} style="width:40%">
  <div class="overlay">&#10084; ${product.params.likeCount}</div>
  <div class="overlay is-new" id="is-new">${desiredDataIsNew}</div>
  <h3 class="product-name">${product.name}</h3>
  <div class="product-categories"><p id="category">${desiredData}</p></div>
  <div class="price-div">
  <span class="price">${product.priceText}</span>
  <del class="price old-price">${product.oldPriceText}</del>
  </div>
  <p class="description">${product.params.basePrice}</p>
</div>
</div>`
  }

  productsData.innerHTML = dataHtml;
}
